...
import { SMA, EMA, MACD, RSI, BollingerBands, ROC } from 'technicalindicators';
...
export class TechnicalAnalysisService {
  ...
  async analyze(symbol: string) {
    ...
    try {
      const parsed = rawPrices?.[upperSymbol]
        ? JSON.parse(rawPrices[upperSymbol])
        : null;
    ...
    const sma50arr =
      prices.length >= 50 ? SMA.calculate({ period: 50, values: prices }) : [];
    const sma200arr =
      prices.length >= 200
        ? SMA.calculate({ period: 200, values: prices })
        : [];
    ...
    const macdArr =
      prices.length >= 35
        ...
        : [];
    const rsiArr =
      prices.length >= 14 ? RSI.calculate({ period: 14, values: prices }) : [];
    const bbArr =
      prices.length >= 20
        ? BollingerBands.calculate({ period: 20, stdDev: 2, values: prices })
        : [];
    const rocArr =
      prices.length >= 10 ? ROC.calculate({ period: 10, values: prices }) : [];
    ...
    const bbPercentB =
      bb != null
        ...
        : null;
    const bbBandwidth =
      bb != null ? ((bb.upper - bb.lower) / bb.middle) * 100 : null;
    ...
    if (sma50arr.length >= 20 && sma200arr.length >= 20) {
      ...
      for (
        let i = 1;
        i < Math.min(20, sma50arr.length, sma200arr.length);
        i++
      ) {
        const cur50 = sma50arr[sma50arr.length - 1 - i];
        const prev50 = sma50arr[sma50arr.length - 2 - i];
        const cur200 =
          sma200arr[sma200arr.length - 1 - i + offset200 - offset50];
        const prev200 =
          sma200arr[sma200arr.length - 2 - i + offset200 - offset50];
    ...
    const allSupports = cluster(supports)
      .filter((s) => s < currentPrice)
      .sort((a, b) => b - a)
      .slice(0, 3);
    const allResistances = cluster(resistances)
    ...
    if (rsi != null) {
      if (rsi < 30) {
        score += 20;
        basis.push('RSI oversold (buy signal)');
      } else if (rsi > 70) {
        score -= 20;
        basis.push('RSI overbought (sell signal)');
      } else {
        basis.push(`RSI neutral at ${rsi.toFixed(1)}`);
      }
    }
    if (sma20 != null) {
      if (currentPrice > sma20) {
        score += 10;
        basis.push('Price above SMA20');
      } else {
        score -= 10;
        basis.push('Price below SMA20');
      }
    }
    if (sma50 != null) {
      if (currentPrice > sma50) {
        score += 15;
        basis.push('Price above SMA50');
      } else {
        score -= 15;
        basis.push('Price below SMA50');
      }
    }
    if (sma200 != null) {
      if (currentPrice > sma200) {
        score += 20;
        basis.push('Price above SMA200 (long-term bullish)');
      } else {
        score -= 20;
        basis.push('Price below SMA200 (long-term bearish)');
      }
    }
    if (macd?.MACD != null && macd?.signal != null) {
      if (macd.MACD > macd.signal) {
        score += 15;
        basis.push('MACD above signal (bullish cross)');
      } else {
        score -= 15;
        basis.push('MACD below signal (bearish)');
      }
    }
    if (goldenCross) {
      score += 20;
      basis.push('Golden Cross recently detected');
    }
    if (deathCross) {
      score -= 20;
      basis.push('Death Cross recently detected');
    }
    ...
    const confidence: Confidence =
      rsi != null && sma200 != null && macd != null
        ? 'High'
        : rsi != null && sma50 != null
          ? 'Medium'
          : 'Low';
    ...
    const chartData = history.slice(startIdx).map((h, i) => {
      ...
      const macdVal =
        absIdx >= macdOffset ? (macdArr[absIdx - macdOffset] ?? null) : null;
      ...
      return {
        ...
        bollingerUpper:
          bbVal != null ? parseFloat(bbVal.upper.toFixed(2)) : null,
        bollingerMiddle:
          bbVal != null ? parseFloat(bbVal.middle.toFixed(2)) : null,
        bollingerLower:
          bbVal != null ? parseFloat(bbVal.lower.toFixed(2)) : null,
        macdValue:
          macdVal?.MACD != null ? parseFloat(macdVal.MACD.toFixed(4)) : null,
        macdSignal:
          macdVal?.signal != null
            ? parseFloat(macdVal.signal.toFixed(4))
            : null,
        macdHistogram:
          macdVal?.histogram != null
            ? parseFloat(macdVal.histogram.toFixed(4))
            : null,
    ...
    return {
      ...
      indicators: {
        ...
        macd: {
          ...
          signal:
            macd?.signal != null ? parseFloat(macd.signal.toFixed(4)) : null,
          histogram:
            macd?.histogram != null
              ? parseFloat(macd.histogram.toFixed(4))
              : null,
          trend: macdTrend,
        },
        rsi14: {
        ...
        bollingerBands: {
          ...
          bandwidth:
            bbBandwidth != null ? parseFloat(bbBandwidth.toFixed(2)) : null,
          percentB:
            bbPercentB != null ? parseFloat(bbPercentB.toFixed(2)) : null,
        },
        roc10: roc10 != null ? parseFloat(roc10.toFixed(4)) : null,
        momentum20:
          momentum20 != null ? parseFloat(momentum20.toFixed(2)) : null,
      },
      trendAnalysis: {
        ...
        priceVsSma20:
          sma20 != null
            ? parseFloat((((currentPrice - sma20) / sma20) * 100).toFixed(2))
            : null,
        priceVsSma50:
          sma50 != null
            ? parseFloat((((currentPrice - sma50) / sma50) * 100).toFixed(2))
            : null,
        priceVsSma200:
          sma200 != null
            ? parseFloat((((currentPrice - sma200) / sma200) * 100).toFixed(2))
            : null,
        goldenCross,
        deathCross,
      },
